...
The command fails instead of deriving missing values. The cutover inputs file must explicitly provide the non-legacy values required for the microservice release, including `noebs.default_tenant_id`, API gateway admin credentials, SMS provider values, Google OAuth redirect URL, Keycloak bootstrap/database credentials, card-vault data key, PSP provider secrets, and resolved EBS endpoint/IPIN/app-id fields. The legacy root supplies only non-empty values that already exist there, such as the Postgres password inside `noebs.db_url`, JWT secret, and Google OAuth client ID/secret. The cutover input schema is strict, so stale duplicated keys such as Google OAuth client ID/secret are rejected instead of ignored.
