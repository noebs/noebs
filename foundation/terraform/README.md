...
the `edge` namespace because it owns the host listeners on 80 and 443 and
...
Before the edge Application rolls out, render and apply its dynamic CA-only
Secret from the validated release: `noebs render-keycloak-transport-ca
/path/to/noebs-kubernetes-release edge | kubectl apply -f -`. This Secret is
labeled as release-renderer-owned and contains no private key; Argo owns the
Deployment and content-addressed Caddy configuration, not this release CA.
...
Before syncing the Argo CD application, render the runtime Kubernetes Secrets from a Kubernetes release directory. The directory must use the same file layout as the in-cluster preflight mount: `config.yaml`, `tenant-catalog.yaml`, `release-manifest.yaml`, runtime and migration role files under `services/*.yaml`, service secret files under `secrets/*.secrets.yaml`, `.sops/age-key.txt`, and platform files under `platform/`. Extra top-level, `.sops`, platform, service config, or service secret entries are rejected so stale inputs cannot be carried into a Kubernetes release.
...
The input is the sole release authority. It explicitly supplies database credentials, the realm-local reconciler and confidential back-office client credentials, gateway encryption keyring, workload signing keys, transport CA, Google identity-provider credentials, GHCR Docker config, card-vault key, opaque PSP callback IDs and provider secrets, and resolved EBS values. Missing values fail preparation; normal preparation never generates or recovers authority from another root.
