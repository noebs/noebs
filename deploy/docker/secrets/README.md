...
`ebs-adapter.secrets.yaml` must carry explicit resolved EBS runtime values: `consumer_endpoint`, `merchant_endpoint`, `ipin_endpoint`, `consumer_app_id`, and `merchant_app_id`. Do not provide QA/prod pairs or mode booleans and expect the runtime to choose.
