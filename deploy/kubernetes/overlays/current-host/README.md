...
The renderer first validates the Kubernetes release layout, then emits Kubernetes `Secret` manifests. The input directory must contain `config.yaml`, one `services/<role>.yaml` file for every runtime and migration role mounted by the preflight Job, one `secrets/<service>.secrets.yaml` file per service-owned secret, `.sops/age-key.txt`, and explicit platform files under `platform/`: `postgres-password.txt`, `temporal-postgres-password.txt`, `keycloak-postgres-password.txt`, and `keycloak.conf`. Missing files, extra service config or service secret YAML files, placeholders, invalid TLS material, and incomplete service database/EBS/Keycloak inputs fail before any manifest is written.
