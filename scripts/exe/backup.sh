...
print(value['cold_volumes']['noebs-mojaloop-redis'])
PY
)
stamp=${fields[0]}
...
if [[ "$mode" == snapshot ]]; then
  ...
  mkdir -m 0700 "$work"
  for name in kafka noebs-mojaloop-redis; do
  ...
  tar czf - -C "${fields[2]}" . | /usr/local/bin/age "${age_recipients[@]}" -o "$work/$stamp-mojaloop.tar.gz.age"
