...
func TestPSPWebhookAuthModeConstraintsAreExact(t *testing.T) {
	...
	if err := MigrateScope(ctx, db, MigrationScopeWalletLedger); err != nil {
	...
	if _, err := db.ExecContext(ctx, `
		INSERT INTO psp_configs(
			...
			supports_deposit, deposit_response_mapping
		) VALUES
			('tenant-webhook-auth', 'signature-provider', 'Signature Provider', 'https://signature.example', 'Idempotency-Key', ARRAY['SDG'], FALSE, '{}'),
			('tenant-webhook-auth', 'ip-provider', 'IP Provider', 'https://ip.example', 'Idempotency-Key', ARRAY['SDG'], FALSE, '{}')
	`); err != nil {
