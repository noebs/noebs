...
Initial package owner: existing user/auth code in `consumer` plus `store` user/auth tables. The first split owns login, registration, OTP generation/login/verification, Google OAuth, profile, language, password, API-key generation, KYC/check-user, one-time recovery credential issuance, session-epoch validation, and device-token update routes. Recovery credentials are opaque, expire after ten minutes, are stored only as digests, and can rotate a password and device public key but cannot authenticate a normal route. A successful recovery increments the identity session epoch; the API gateway validates that epoch with identity-auth and fails closed, immediately revoking pre-recovery bearer sessions. `check_user` owns user existence checks in identity-auth but obtains masked PAN values through a card-vault command rather than reading card tables or identity-owned card columns. Completed card registration, `register_with_card`, and account-recovery balance commands into identity-auth carry only identity fields; PAN and expiry are persisted only through card-vault. User creation persists identity-auth state only; it does not emit hard-coded external sync or backup calls. Identity-auth exposes one public signup OTP generation route; the legacy `/consumer/otp/generate_insecure` route is not part of the service or gateway surface.
...
Initial package owner: `dashboard` plus wallet admin templates as a transitional read surface. The service exposes dashboard read routes only; transaction projections arrive through the `admin-reporting-projector` Kafka consumer role and not through an internal HTTP write endpoint.
...
- `kafka.noebs.svc.cluster.local:9092`
...
Kubernetes NetworkPolicies restrict ingress by exact pod role labels. Noebs Postgres only accepts database traffic from database-opening service roles and their migration Jobs. Kafka only accepts traffic from the topic Job, the EBS publisher roles, and the admin-reporting projector. Temporal Postgres only accepts Temporal runtime and schema migration traffic. Temporal frontend only accepts workflow clients. Keycloak Postgres only accepts Keycloak. Public HTTP service pods only accept HTTP traffic from `api-gateway` plus their explicit internal command callers: `identity-auth` accepts `ebs-adapter`, `card-vault` accepts `identity-auth` and `ebs-adapter`, and `notification-chat` accepts `ebs-adapter`. `wallet-ledger` accepts gRPC traffic only from `wallet-api`.
...
1. Add explicit config-selected runtime roles to the current binary: `api-gateway`, `identity-auth`, `card-vault`, `ebs-adapter`, `ebs-adapter-events`, `psp-webhook`, `admin-reporting`, `admin-reporting-projector`, `notification-chat`, `consumer-beneficiary`, `wallet-api`, `wallet-ledger`, and `wallet-worker`.
...
13. Move admin/reporting to event-driven projections. `ebs-adapter-events` publishes EBS transaction outbox rows to Kafka, and `admin-reporting-projector` applies those messages idempotently into the admin-reporting projection table. Payment writes do not call reporting code.
