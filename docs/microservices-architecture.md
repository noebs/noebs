...
Kubernetes NetworkPolicies restrict ingress to the shared platform backends by exact pod role labels. Noebs Postgres only accepts database traffic from database-opening service roles and their migration Jobs. Kafka only accepts traffic from the topic Job, the EBS publisher roles, and the admin-reporting projector. Temporal Postgres only accepts Temporal runtime and schema migration traffic. Temporal frontend only accepts workflow clients. Keycloak Postgres only accepts Keycloak.
