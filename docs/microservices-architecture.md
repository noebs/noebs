...
Initial package owner: card/token functions currently in `consumer` and `store`. The first split owns card CRUD, main-card selection, payment-token storage, payment-request token creation, internal quick-pay token resolution/paid-marking commands using the gateway-propagated user ID rather than identity-owned `users` rows, internal completed-registration card persistence, and mobile-to-card lookup through an explicit identity-auth user-ID resolution command.
