...
Initial package owner: existing user/auth code in `consumer` plus `store` user/auth tables. The first split owns login, registration, OTP, Google OAuth, profile, language, password, API-key generation, KYC/check-user, and device-token update routes. `check_user` owns user existence checks in identity-auth but obtains masked PAN values through a card-vault command rather than reading card tables or identity-owned card columns. Completed card registration and `register_with_card` commands into identity-auth carry only identity fields; PAN and expiry are persisted only through card-vault.
