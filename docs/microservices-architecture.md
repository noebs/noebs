...
Initial package owner: `dashboard` plus wallet admin templates as a transitional read surface. The service exposes dashboard read routes only; transaction projections must arrive through the configured Kafka topic and not through an internal HTTP write endpoint.
