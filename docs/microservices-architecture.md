...
Kubernetes runtime Secrets are materialized from an explicit Kubernetes release input directory with `noebs render-kubernetes-secrets /path/to/noebs-kubernetes-release noebs /path/to/tls.crt /path/to/tls.key | kubectl apply -f -`. The renderer validates the Kubernetes preflight layout before output: `config.yaml`, the exact runtime and migration role files under `services/`, the exact service secret files under `secrets/`, `.sops/age-key.txt`, and platform files under `platform/`. Extra service config or service secret YAML files are rejected so stale monolith or compatibility inputs cannot be ignored. It copies service SOPS files into service-owned Kubernetes Secrets, reads Postgres/Temporal/Keycloak platform passwords from explicit files, includes the SOPS age key, and renders `noebs-tls` from explicit TLS files. Generated Secret manifests are deployment output and must not be committed.
