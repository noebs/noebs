...
(cd "$repo/infra/exedev-provider" && go build -trimpath -o "$provider_dir/terraform-provider-exedev_v0.1.0" .)
cp "$repo/infra/exedev/main.tf" "$state/main.tf"
...
export EXEDEV_TOKEN
EXEDEV_TOKEN=$(python3 "$repo/infra/scripts/api-token.py" "$EXE_SSH_KEY")
