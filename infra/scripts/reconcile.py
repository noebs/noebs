...
def ssh_args(key, destination):
    return ["ssh", "-i", str(key), "-o", "IdentitiesOnly=yes", "-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=yes",
            "-o", "UserKnownHostsFile="+str(ROOT/'infra/exe/known_hosts'), "-o", "ConnectTimeout=20", "-o", "ConnectionAttempts=3",
...
def main():
    ...
    fleet = json.loads((ROOT / "infra/exedev/fleet.json").read_text())
    ...
    run([str(ROOT / "infra/scripts/install-terraform.sh"), str(work / "bin")])
    ...
    if not controllers:
        ...
        run([str(ROOT / "infra/scripts/terraform.sh"), "apply", "-input=false", "-auto-approve", "-no-color", "-var-file=" + str(control_vars)], env=env)
    ...
    run(["go", "build", "-trimpath", "-o", str(provider), "."], cwd=ROOT / "infra/exedev-provider", env=os.environ | {"CGO_ENABLED": "0", "GOOS": "linux", "GOARCH": "amd64"})
    files = {"terraform": work / "bin/terraform", "terraform-provider-exedev_v0.1.0": provider, "main.tf": ROOT / "infra/exedev/main.tf", "fleet.json": ROOT / "infra/exedev/fleet.json"}
    archive = io.BytesIO()
    ...
    token = run(["python3", str(ROOT / "infra/scripts/api-token.py"), str(key)], capture_output=True).stdout
