...
'''
    ...
    ssh(key,data,'sudo tee /usr/local/sbin/noebs-backup >/dev/null; sudo chmod 0755 /usr/local/sbin/noebs-backup',input=(ROOT/'infra/scripts/backup.sh').read_bytes())
    ssh(key,data,'sudo tee /usr/local/lib/noebs-backup-checkpoint.py >/dev/null',input=(ROOT/'infra/scripts/backup_checkpoint.py').read_bytes())
    lease.check()
