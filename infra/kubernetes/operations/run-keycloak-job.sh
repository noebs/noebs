...
if [[ $# -ne 1 || ("$1" != lookup && "$1" != dry-run && "$1" != apply) ]]; then
  echo "usage: $0 <lookup|dry-run|apply>" >&2
  exit 2
fi
...
steady_root="$repo_root/infra/kubernetes/overlays/exe"
if [[ "$mode" == lookup ]]; then
  operation_root="$script_dir/lookup"
  job_name=noebs-keycloak-subject-lookup
else
  operation_root="$script_dir/memberships/$mode"
  job_name=noebs-keycloak-membership-assignment
fi
authority_root="$repo_root/infra/kubernetes/keycloak-authority"
...
release="$("${kubectl_cmd[@]}" -n noebs get configmap noebs-release -o json)"
jq -e --arg revision "$head_revision" '
  .data.revision == $revision
  and .data.stage == "production"
  and (.data.image | test("^ghcr[.]io/noebs/noebs@sha256:[0-9a-f]{64}$"))
' <<<"$release" >/dev/null
release_image="$(jq -r '.data.image' <<<"$release")"
"${kubectl_cmd[@]}" -n noebs rollout status deployment/keycloak --timeout=120s
...
extract_object "$operation_render" Job "$job_name" exact "$job_manifest"
python3 - "$job_manifest" "$release_image" <<'PYJOB'
import pathlib
import re
import sys
import yaml
...
for container in spec['containers'] + spec.get('initContainers', []):
    if not re.fullmatch(r'ghcr[.]io/noebs/noebs@sha256:[0-9a-f]{64}', container['image']):
        raise SystemExit('operation contains an unexpected image authority')
