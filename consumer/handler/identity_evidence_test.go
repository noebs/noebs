...
func TestIdentityEvidenceRejectsInvalidInputAtBoundary(t *testing.T) {
	...
	for _, payload := range []string{
		`{"session_id":"` + identityTestID + `","document_type":"passport","previous_session_id":"bad-id"}`,
...
func TestIdentityEvidenceReceiptNeverClaimsVerification(t *testing.T) {
	...
	if receipt.Status != "submitted" || receipt.LivenessStatus != "not_evaluated" ||
		receipt.DocumentStatus != "not_evaluated" || receipt.ReviewStatus != "pending_review" {
...
func TestIdentityEvidenceCustomerCreateAcceptsAbsentAndLegacyMarker(t *testing.T) {
	app := fiber.New()
	app.Use(authenticatedUserTestIdentity)
	RegisterIdentityAuthedRoutes(app.Group("/consumer"), &Handler{})
	for _, marker := range []string{"", `,"synthetic":false`, `,"synthetic":true`} {
		payload := `{"session_id":"` + identityTestID + `","document_type":"passport"` + marker + `}`
		response, err := app.Test(httptest.NewRequest(http.MethodPost, "/consumer/identity/sessions", bytes.NewBufferString(payload)))
		if err != nil {
			t.Fatal(err)
		}
		// No service is installed: an unavailable response proves the boundary
		// accepted this ordinary client shape rather than demanding a declaration.
		assertJSONStatusCode(t, response, http.StatusInternalServerError, "identity_unavailable")
		_ = response.Body.Close()
	}
}

func TestIdentityEvidenceManualApprovalDoesNotClaimAutomatedChecks(t *testing.T) {
	app := fiber.New()
	app.Get("/", func(c *fiber.Ctx) error {
		return identityResult(c, store.IdentitySession{Status: "approved", Review: &store.IdentityReview{Decision: "approved", Method: "manual", Reason: "Account evidence accepted by the review team."}}, nil)
	})
	response, err := app.Test(httptest.NewRequest(http.MethodGet, "/", nil))
	if err != nil {
		t.Fatal(err)
	...
	if receipt.ReviewStatus != "approved" || receipt.Review == nil || receipt.Review.Method != "manual" || receipt.LivenessStatus != "not_evaluated" || receipt.DocumentStatus != "not_evaluated" {
		t.Fatalf("manual approval fabricated verification: %+v", receipt)
	}
}
