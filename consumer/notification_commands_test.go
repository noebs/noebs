...
func TestNotificationTransactionUUIDIsDistinctAndTapAuthorizationUsesParticipants(t *testing.T) {
	db, storeSvc, tenantID := newTestDBWithScopes(t, []string{
	...
	stmt := db.Rebind(`SELECT uuid, ebs_uuid FROM push_data WHERE tenant_id = ? AND uuid = ?`)
	if err := db.GetContext(context.Background(), &stored, stmt, tenantID, transactionUUID+":sender"); err != nil {
		t.Fatalf("read persisted notification: %v", err)
	}
	if stored.UUID != transactionUUID+":sender" || stored.EBSUUID != transactionUUID {
		t.Fatalf("stored notification identities = %+v", stored)
	}
	if _, err := service.GetTransactionByUUIDForUser(context.Background(), tenantID, 42, stored.EBSUUID); err != nil {
		t.Fatalf("participant notification tap: %v", err)
	}
	if _, err := service.GetTransactionByUUIDForUser(context.Background(), tenantID, 84, stored.EBSUUID); !errors.Is(err, ErrTransactionNotFound) {
